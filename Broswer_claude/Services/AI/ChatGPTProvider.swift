...
// Implementation of AIProvider using the OpenAI Chat Completions API.
...
//    4. action=confirm → required before any destructive action.
//    5. NEVER use your training knowledge for price/dates. Extract what is ON SCREEN.
//    6. Card with DISABLED button = current plan.
//    """
...
//        Reply with ONLY valid JSON: {"serviceName": "...", "intent": "get_details|cancel"}
//        Message: "\(input)"
//        """
        ...
        let body: [String: Any] = [
            ...
            "messages": [
                ["role": "system", "content": "Find the service name for: \(input)"],
        ...
        dlog("[ChatGPT] Parsed intent: service=\(serviceName ?? "nil"), intent=\(intent ?? "nil")", tag: "AI")
    ...
    func resolveStartURL(service: String) async throws -> ServiceURLs {
        ...
        """
//        let prompt = "Return JSON: {\"displayName\": \"name\", \"loginURL\": \"url\"} for the official login page of: \(service)"
        
        let body: [String: Any] = [
            "model"      : Config.chatGptModel,
            "max_completion_tokens" : 200,
            "messages"   : [
                ["role": "system", "content": "What is the official login page URL for: \(service)"],
                ["role": "user", "content": prompt]
            ]
        ]
        ...
        dlog("[ChatGPT] Resolved login URL for \(service): \(loginURL ?? "nil")", tag: "AI")
    ...
    func resolveBillingURL(service: String) async throws -> ServiceURLs {
        let prompt = "Return JSON: {\"displayName\": \"name\", \"billingURL\": \"url\"} for the official billing page of: \(service)"
        
        let body: [String: Any] = [
            "model"      : Config.chatGptModel,
            "max_completion_tokens" : 200,
            "messages"   : [
                ["role": "system", "content": "You are a URL expert. Return JSON ONLY."],
                ["role": "user", "content": prompt]
            ]
        ]
        ...
        let displayName = obj["displayName"] as? String ?? service.capitalized
        dlog("[ChatGPT] Resolved billing URL for \(service): \(billingURL ?? "nil")", tag: "AI")
    ...
    private func buildContent(task: AgentTask, currentURL: String, snapshot: PageSnapshot, screenshot: UIImage?, history: [String], knownRoute: LearnedRoute?, collectedSoFar: String, stepNumber: Int) -> [[String: Any]] {
        ...
        // Text part first for GPT-4o typically
        let memoryHint = knownRoute?.promptHint() ?? ""
        let recentHistory = history.suffix(5)
        let prompt = "GOAL: \(task.goalDescription)\nURL: \(currentURL)\nCOLLECTED: \(collectedSoFar)\nSTEP: \(stepNumber + 1)\n\n\(memoryHint)\n\(snapshot.formatted())\n\nRECENT: \(recentHistory.joined(separator: "\n"))\n\nRespond with JSON only."
    ...
    private func post(_ body: [String: Any]) async throws -> Data {
        ...
        req.setValue("Bearer \(Config.chatGptAPIKey)", forHTTPHeaderField: "Authorization")
        req.httpBody = bodyData
        req.timeoutInterval = 120 // Increase to 2 minutes for vision/heavy tasks
        
        do {
            let (data, resp) = try await URLSession.shared.data(for: req)
            guard let http = resp as? HTTPURLResponse else {
                throw AgentError.apiError("No HTTP response")
            }
            ...
            let errorMsg = "[ChatGPT] HTTP \(http.statusCode): \(bodyString)"
            print(errorMsg)
            dlog(errorMsg, tag: "ERROR")
            
            throw AgentError.apiError(errorMsg)
        } catch let error as URLError {
            let msg = "[ChatGPT] Network Error: \(error.localizedDescription) (code: \(error.code.rawValue))"
            print(msg)
            dlog(msg, tag: "ERROR")
            throw AgentError.apiError(msg)
        } catch {
            let msg = "[ChatGPT] Unexpected Error: \(error.localizedDescription)"
            print(msg)
            dlog(msg, tag: "ERROR")
            throw AgentError.apiError(msg)
        }
    ...
    private func parseJSONResponse(_ data: Data) throws -> [String: Any] {
        guard
            ...
            throw AgentError.parseError("GPT Parse Error: \(errorMsg)") 
        }
        
        let jsonStr = extractJSON(from: text)
        guard let jsonData = jsonStr.data(using: .utf8),
              let obj = try? JSONSerialization.jsonObject(with: jsonData) as? [String: Any]
        else { throw AgentError.parseError("Invalid JSON in GPT response: \(text)") }
        return obj
    }
