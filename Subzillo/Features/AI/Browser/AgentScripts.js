...
window.SubzilloAgent = {
    ...
    extractSnapshot: function () {
        ...
        function collect(root) {
            ...
            for (var i = 0; i < els.length; i++) {
                ...
                if (tagName === 'iframe') {
                    ...
                    elements.push({ text: '[' + label + ']', href: '', selector: 'iframe' + (el.id ? '#' + el.id : ''), priority: 0 });
                ...
                // PRIORITY BOOST: Login, Sign In, My Account, Menu
                var priority = 0;
                var lowText = text.toLowerCase();
                if (lowText.includes('sign in') || lowText.includes('login') || lowText.includes('account') || lowText.includes('menu')) {
                    priority = 10;
                }
        ...
        // Sort by priority (highest first) then original order
        elements.sort((a, b) => b.priority - a.priority);

        return {
            text: pageText,
            elements: elements.slice(0, 150), // Increase limit for more visibility
    ...
    click: function (selector, clickText) {
        function find(root) {
            ...
            if (lowText) {
                ...
                while (walker.nextNode()) {
                    ...
                    var content = (node.innerText || node.value || node.getAttribute('aria-label') || node.getAttribute('title') || "").trim().toLowerCase();
        ...
        // THE EVENT HAMMER (Sequential for human-like behavior)
        function dispatch(type, x, y) {
            try {
                var ev;
                var props = { bubbles: true, cancelable: true, view: window, composed: true, clientX: x, clientY: y, screenX: x, screenY: y };
                if (type.startsWith('touch')) ev = new TouchEvent(type, props);
                else if (type.startsWith('pointer')) ev = new PointerEvent(type, Object.assign(props, { isPrimary: true, pointerId: 1 }));
                else ev = new MouseEvent(type, props);
